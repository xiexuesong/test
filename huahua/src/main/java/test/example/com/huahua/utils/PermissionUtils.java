...
public class PermissionUtils {
     ...
     * 判断是否给予权限
     */
    public boolean judgePermissions(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            boolean flag1 = activity.shouldShowRequestPermissionRationale(Manifest.permission.READ_EXTERNAL_STORAGE);//返回false 有可能是 禁止后不在显示 还有可能是允许不再显示
            if (flag1) {
                ...
                if (ActivityCompat.checkSelfPermission(activity, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(activity, "已禁用读取权限，请手动授予", Toast.LENGTH_SHORT).show();
                    return false;
                }
            }
        }
